...
dataDirectory <- paste0( baseDirectory, "../Data/SimulationExperiments_Noise_NoN4")
...
for( l in seq.int( length( diagnosisType ) ) )
  ...
  for( p in seq.int( length( pipelineType ) ) )
    ...
    originalFiles <- list.files( path = dataDirectory,
      pattern = paste0( "*OriginalImage.nii.gz" ),
      recursive = TRUE, full.names = TRUE )

    segmentationFiles <- c()
    for( q in seq.int( length( originalFiles ) ) )  
      {
      segmentationFiles <- append( segmentationFiles, sub( "OriginalImage", pipelineType[p], originalFiles[q] ) )
      }
  ...
  ggsave( paste0( figuresDirectory, "/volumeXNoN4RocDx", diagnosisType[l], ".pdf" ), g, width = 5, height = 5, units = "in" )
